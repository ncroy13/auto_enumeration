...
portscan(){
    nmap --open -iL nmap/alive.ip \
	 -sTU -T4 -A -Pn -n -oA nmap/scans/portscan -v \
...
#summary(){
#    echo -e "\n[${GREEN}+${RESET}] there are $(cat ./nmap/alive.ip | wc -l ) ${YELLOW}alive hosts${RESET} and $(egrep -o '[0-9]*/open/' nmap/scans/portscan.gnmap | sort | uniq | wc -l) ${YELLOW}unique ports/services${RESET}"
#    for ip in $(cat ./nmap/alive.ip); do
#	echo -e $ip > ./nmap/open-ports/$ip.txt
#	awk \/$ip\/ nmap/scans/portscan.gnmap | egrep -o '*[0-9]*/open/*[tcp/udp]*/' | sort | uniq | awk -F '/' '{print $1"/"$3}' >> ./nmap/open-ports/$ip.txt
#    done
#}
...
#nmappingsweep
#echo -e "\n[${GREEN}+${RESET}] running an ${YELLOW}nmap port scan${RESET} for all ip in alive.ip"
#nmapportscan
